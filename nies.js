(function () {
  ...
  function hasNeisTransport() {
    return Boolean(config.apiKey || proxyBase);
  }
  ...
  async function resolveSchool(user) {
    ...
    const schools = await searchSchool(config.schoolName);
    const school = schools.find((item) => item.name === config.schoolName) || schools[0];

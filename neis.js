...
(function () {
    ...
    function createUrl(endpoint, params = {}, options = {}) {
        ...
        if (config.apiKey)
            requestParams.KEY = config.apiKey;
        ...
        if (!response.ok) {
            ...
            if (shouldFallbackToDirect(response, message)) {
                ...
                if (response.ok) {
                    ...
                    return readRowsOrThrow(data, rowKey, {
    ...
    async function searchSchool(name) {
        if (!hasNeisTransport())
            throw new Error("NEIS 연동 설정이 없습니다.");
    ...
    async function getMeal(user) {
        ...
        const rows = await request("mealServiceDietInfo", {
    ...
    async function getTimetable(user) {
        ...
        const weekdayRows = await Promise.all(weekDates.map((date) => request("hisTimetable", {
            ATPT_OFCDC_SC_CODE: school.office,
            SD_SCHUL_CODE: school.code,
    ...
    function applyTimetable(rows, options = {}) {
        ...
        if (!options.preserveExisting) {
            document.querySelectorAll("tbody tr[data-period]")
                .forEach((tableRow) => {
                if (!isPeriodName(tableRow.dataset.period))
                    return;
